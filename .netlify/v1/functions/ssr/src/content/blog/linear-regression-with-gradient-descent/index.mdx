...
we can express $\sum_{i=1}^{n} (y_{i} - \hat{y_{i}})^{2}$ &emsp; as ↴

$$(y_{i} - \hat{y_{i}})^2\ =\ (y - \hat{y})^T(y - \hat{y})$$

$$(y - \hat{y} )^T(y - \hat{y} ) =\ (y - X\hat{B} )^T(y - X\hat{B} )$$

$$ =\ (y^{T} - (X\hat{B})^{T})(y - X\hat{B})$$

$$ =\ (y^{T} - \hat{B}^{T}X^{T})(y - X\hat{B})$$

$$ = y^{T}y - y^{T}X\hat{B} - \hat{B}^{T}X^{T}y + \hat{B}^{T}X^{T}X\hat{B}$$

$$\frac{\partial\ ({(y - X\hat{B})^T(y - X\hat{B})}) }{\partial \hat{B}} = 0$$

$$\Rightarrow \quad \frac {\partial\ (y^{T}y\ -\ y^{T}X\hat{B}\ -\ \hat{B}^{T}X^{T}y\ +\ \hat{B}^{T}X^{T}X\hat{B})}{\partial \hat{B}} = 0$$

$$\Rightarrow \frac{\partial y^{T}y}{\partial \hat{B}} - \frac{\partial  y^{T}X\hat{B}}{\partial \hat{B}} - \frac{\partial \hat{B}^{T}X^{T}y}{\partial \hat{B}} + \frac{\partial \hat{B}^{T}X^{T}X\hat{B}}{\partial \hat{B}} = 0$$

if we have a matrix $A_{m \times m}$ and a vector $x_{m \times 1}$, then:

$$\large \frac{\partial Ax}{\partial x} = A, \ \frac{\partial x^{T}A}{\partial x} = A^{T}, \ \frac{\partial x^{T}Ax}{\partial x} = 2A^{T}x$$
...
$$\therefore \quad -y^{T}X - y^{T}X + 2X^{T}X\hat{B} \quad = \quad 0$$
