...
const $$AboutExperience = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="relative flex flex-col justify-start pl-12"> <div class="absolute top-0 left-0 z-40 flex items-center justify-center -translate-x-1/2 bg-white dark:bg-neutral-950 w-14 h-14 rounded-full border-2 border-violet-200 dark:border-fuchsia-800/60 ring-2 ring-offset-2 ring-violet-100 dark:ring-fuchsia-900/40 ring-offset-white dark:ring-offset-neutral-950"> <img${addAttribute(logo, "src")}${addAttribute(company, "alt")} class="w-8 h-8 dark:invert"> </div> <p class="text-xs uppercase text-neutral-400 dark:text-neutral-500 trackign-widest"> ${dates} </p> <h3 class="my-1 text-lg font-bold dark:text-neutral-100">${role}</h3> <p class="mb-1 text-sm font-medium dark:text-neutral-300">${company}</p> <p class="text-sm font-light text-neutral-600 dark:text-neutral-400"> ${description} </p> </div>`;
...
</span> </div> </div> <!-- Bio --> <div class="border-l-2 border-violet-500 dark:border-fuchsia-500 pl-6 mb-4"> <div class="text-sm leading-6 text-gray-600 dark:text-neutral-300 sm:leading-7 lg:leading-8 sm:text-base lg:text-lg text-justify"> ${renderComponent($$result2, "AboutMeContent", Content, {})} </div> </div> ${renderComponent($$result2, "Separator", $$Separator, { "text": "Technical Skills" })} <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4"> ${tools.map((skill) => renderTemplate`<div class="group rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-sm dark:shadow-none p-5 hover:border-violet-300 dark:hover:border-fuchsia-800/60 hover:shadow-md hover:shadow-violet-500/10 transition-all duration-300"> <div class="flex items-center justify-between mb-4"> <span class="font-mono text-xs font-semibold uppercase tracking-wider text-violet-600 dark:text-fuchsia-400"> ${skill.skillName} </span> <span class="font-mono text-[10px] text-neutral-300 dark:text-neutral-600 tabular-nums"> ${skill.skillTools.length.toString().padStart(2, "0")} </span> </div> <div class="flex flex-wrap gap-2"> ${skill.skillTools.map((tool) => renderTemplate`<a${addAttribute(tool.url, "href")} target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-950 hover:border-violet-300 dark:hover:border-fuchsia-700/50 hover:bg-violet-50 dark:hover:bg-fuchsia-900/10 transition-all duration-150"> ${renderComponent($$result2, "Icon", $$Icon, { "name": tool.logo, "size": 16 })} <span class="text-[11px] font-medium text-neutral-600 dark:text-neutral-400"> ${tool.tooltip} </span> </a>`)} </div> </div>`)} </div> ${renderComponent($$result2, "Separator", $$Separator, { "text": "Education" })} <div class="px-5 py-10 grid place-content-center"> ${education.map((study, loop) => {
